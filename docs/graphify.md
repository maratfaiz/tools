...
Когда Claude Code разбирается в незнакомом проекте, он обычно грепает файлы, читает их один за другим, пытается собрать в голове картину «что от чего зависит» — и делает это заново в каждой новой сессии, тратя на это кучу токенов и времени.

**Конкретный пример.** Спрашиваешь: «что связывает авторизацию с базой данных?»
- **Без Graphify:** Claude грепает файлы по слову «auth», открывает штук 10-20 файлов один за другим, пытается сам восстановить цепочку вызовов — долго и дорого по токенам, и результат зависит от того, удачно ли он выбрал, что читать.
- **С Graphify:** один раз строишь граф всего проекта (`/graphify .`), и дальше просто спрашиваешь `/graphify path "Auth" "Database"` — получаешь готовый путь по шагам за секунды, потому что связи между всеми частями кода уже посчитаны один раз заранее.

Это как разница между «каждый раз искать дорогу по домам вслепую» и «один раз построить карту города, а потом просто по ней прокладывать маршрут».
